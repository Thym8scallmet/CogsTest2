...
class VotingModal(Modal):
  ...
  def __init__(self, cog):  # Pass the cog instance during initialization
    ...
    self.add_item(
        TextInput(label="Enter the Title of the Poll",
                  placeholder="Enter the title",
                  required=True))
    for i in range(4):
      self.add_item(
          TextInput(label=f"Option {i + 1}",
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    if options:
      poll = Poll(title=title,
                  ...
                  visibility=True,
                  duration=60,
                  message_details=(interaction.channel_id, None))
      view = PollView(poll)
      ...
      await interaction.response.send_message(
          "Please provide options for the poll.", ephemeral=True)
...
def create_poll_embed(poll):
  ...
  for option_id, option in poll.options.items():
    embed.add_field(name=f"Option {option_id}: {option[0]}",
                    value=f'Votes: {option[1]}',
                    inline=False)
  return embed


@app_commands.command(name="create_poll",
                      description="Creates a new interactive poll.")
async def create_poll(interaction: discord.Interaction):
  ...
  if cog:
    ...
    print("VotePoll cog not found."
          )  # Handle the case when VotePoll cog is not found


async def setup(client: commands.Bot):
  vote_poll = VotePoll(client)
  voting_modal = VotingModal(
      vote_poll)  # Pass vote_poll instance to VotingModal
