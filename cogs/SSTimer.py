...
class SSTimer(commands.Cog):
  ...
  @app_commands.command(
      name="ss_timer",
      description="Gives a one minute timer for taking a screenshot")
  async def sstimer(self, interaction: discord.Interaction):
    ...
    msg = await interaction.followup.send(
        "Starting screenshot timer. Get ready to take your screenshot.",
        ephemeral=True)
    ...
    for i in range(remaining_time_for_second_screenshot, 0, -5):
      ...
      if i <= 10:  # Start final countdown when 10 seconds are left, accounting for the final 5s countdown and 2s pause
        break
      await msg.edit(
          content=f"Get ready to take your second screenshot in {i}...")

    await asyncio.sleep(
        2)  # Short wait before the final countdown, adjust this as needed
    ...
    await asyncio.sleep(
        2)  # Wait before final message, adjust to ensure timings match
